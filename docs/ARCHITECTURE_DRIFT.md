...
| 设计意图 | 当前实现 | 分类 | 说明 |
|----------|----------|------|------|
| Memory Agent 抽取事实、建图 | 无 | `technical debt` | `StubEngine` 只写 `episodes_stub.jsonl` 预览行 |
| retrieve 由 Agent 裁切关联记忆 | 占位 `hints` 文案 | `technical debt` | 仅统计 episode 条数 + 最近 preview |
